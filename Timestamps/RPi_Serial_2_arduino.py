...
current_state = ['s', 's']  # current state is the state that it needs to do
while values[0] < 999.99 or values[1] < 999.99:
...
s0_send_times = []
current_state = ['s', 's']  # current state is the state that it needs to do
with open('logfile_serial_2multi.log', 'w') as log_file:
    while values[0] < 999.99:
        # need to keep sending until current state != all s
        for i in range(len(current_state)):
            if current_state[i] == 's':  # keep writing if state is sending
                ports[i].write("{:f}\n".format(values[i]).encode('utf-8'))
